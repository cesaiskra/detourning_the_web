...
function getPoo(q, step){
  var url = 'http://www.ratemypoo.com/xyzzy/search?step=' + step + '&search=' + q;
  jsdom.env(url, function (err, window) {
    ...
    $('center center tr').each(function (){
      ...
      console.log(data['imgSrc'], data['title'] + '.jpg');
    ...
    $('a:contains(Next)').each(function (){
      if (this.href.indexOf('/xyzzy') >= 0){
        // next
      }
    });
